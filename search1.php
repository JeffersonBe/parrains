...
else 
{
	...
	{
        $search = addslashes($_POST['search1']);
        ...
        if(strlen($search) >0) 
		{
			...
			if($ecole=='no')
			{
				$query=$bdd->prepare('SELECT nom,prenom FROM parrains WHERE parrainv<>\'2\' AND nom LIKE :r_search LIMIT 5;');
				$query->execute(array('r_search' => $search.'%'));
			}
			else
			{
				$query=$bdd->prepare('SELECT nom,prenom FROM parrains WHERE parrainv<>\'2\' AND nom LIKE :r_search AND ecole=:r_ecole LIMIT 5;');
			...
			while($answer=$query->fetch())
			{
                ...
                echo utf8_encode('</li><li onclick="fill1(\''.strtoupper($answer['nom']).'\',\''.strtolower($answer['prenom']).'\',1);">'.strtoupper($answer['nom']).' '.strtolower($answer['prenom']).'</li>');
            }
        }
		else 
		{

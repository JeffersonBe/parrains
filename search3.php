...
else 
{
    ...
    if(isset($_POST['search3']))
	{
        ...
        if(strlen($search) >0) 
		{
			...
			if($ecole=='no')
			{
				$query=$bdd->prepare('SELECT nom,prenom FROM fillots WHERE fillotv<>\'2\' AND nom LIKE :r_search LIMIT 5;');
				$query->execute(array('r_search' => $search.'%'));
			}
			else
			{
				$query=$bdd->prepare('SELECT nom,prenom FROM fillots WHERE fillotv<>\'2\' AND nom LIKE :r_search AND ecole=:r_ecole LIMIT 5;');
